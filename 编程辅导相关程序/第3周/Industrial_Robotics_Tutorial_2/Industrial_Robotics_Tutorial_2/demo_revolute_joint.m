% This code allows you to test the behaviour of a HTM subject to a revolute
% joint. A Screw axis representation of a revolute joint has h=0 by
% definition
clear all
close all
h=0.;                   % the pitch of a revolute joint is zero by definition
q = [1 1.5 0];            % vector which defines distance between frame and screw axis
omega = [1 1 2*pi];     % rotation vector omega_hat*theta, defines the axis of rotation and amount of rotation
%-------------------------
% q = [2 0 0];
% omega = [0 0 2*pi];
%------------------------
...
for i=1:n
    ...
    plot3([q(1) T1(1,4)],[q(2) T1(2,4)],[q(3) T1(3,4)],'color','r','linewidth', 1)
    trplot(T1, 'color', 'r')
    pause(0.05)
    %hold off
end
