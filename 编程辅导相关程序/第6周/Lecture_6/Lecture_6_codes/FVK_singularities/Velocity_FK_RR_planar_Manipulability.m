...
theta1 = deg2rad(30);
theta2 = deg2rad(90);
% assigns the rotation of two joint angles in the singular case
%theta1 = deg2rad(45);
%theta2 = deg2rad(0);
...
y0=600;     % location of figure in the screen
...
plot_arrow(x_e(1:2),x_e(1:2)+J(1:2,1), 'r-.', 0.4)
plot_arrow(x_e(1:2),x_e(1:2)+J(1:2,2), 'k-.', 0.4)
drawnow
...
% to draw the manipulability ellipsoids we need to test for all possible
% combinations of theta1_dot and theta2_dot and plot the point of
% application of the end-effector velocity in each of these cases.
% we start by assanging initial value to the two joint velocities:
dot_theta1 = 1;
dot_theta2 = 0;
figure(3)
x0=200;     % location of figure in the screen
...
hold on
view(0,90)
% the following loop tests for a range of combianations of joint velocites
N = 50
for i=1:N
    % to define the tested joint velocites we make them range from -1 to 1
    % and devide this range in N little intervals. For each of them we
    % compute each joint velocity.
    % The following command guarantees that in N iterations of the loop the
    % velocity of each joint varies from 1 to -1, this is made possible by
    % using the cosine and sine functions, as explained in class
    dot_theta1 = cos(2*pi/N*i); % rad/sec 
    dot_theta2 = sin(2*pi/N*i); % rad/sec
    dot_theta = [dot_theta1;dot_theta2];
    figure(3)
    dot_x_e = J*dot_theta; % this computes the end-effector velocity for the newly defined dot_theta vector
    plot_arrow(x_e(1:2),x_e(1:2)+dot_x_e(1:2,1), 'r-.', 0.4)
    plot3([x_e(1) x_e(1)+dot_x_e(1)], [x_e(2) x_e(2)+dot_x_e(2)], [0 0],' r.')
    hold on
    
    figure(4) 
    trplot(eye(4), 'color', 'b')
    plot_arrow([0;0],dot_theta, 'k')
    hold on
end
