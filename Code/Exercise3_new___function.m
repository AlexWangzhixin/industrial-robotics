...
while ~found_valid_beta_dot%若当前beta_dot不满足约束，就持续搜索
    ...
    for i = 1:length(time)
        ...
        % 使用忽略姿态约束的数值解法
        % 注意：我们需要传入空间坐标系下的目标点 p_d_S，而不是 p_d_J1
        % 并且需要传入 Slist, M 和一个初始猜测 theta_curr
        if i == 1
            theta_guess = [0; -0.5]; % 初始猜测
        else
            theta_guess = history.theta(:, end); % 使用上一步的结果作为猜测
        end
        
        % 调用新函数
        theta_sol = NumericalIK_PositionOnly(Slist, M, p_d_S, theta_guess);      
        %速度计算
        ...
        J = JacobianSpace(Slist, theta_curr);
        q_dot = J(4:6, :) \ v_cart_F;
        
        % 检查奇点
        if cond(J) > 1e4%若雅可比矩阵条件数过大，接近奇异
            valid_run = false;
            break;
        end
...
for i = 1:length(time)
    ...
    % Inverse Kinematics，使用函数function theta = analytical_IK(x, y, L1, L2)
    theta_sol = IK_2R_IKinBody(p_d_J1(1), p_d_J1(2), L1, L2);
    ...
    if det(J_linear'*J_linear) < 1e-6
        ...
        theta_dot = pinv(J_linear) * v_cart_F;% 使用伪逆求解关节速度
    end
...
function theta = NumericalIK_PositionOnly(Slist, M, p_target, theta0)
    % 这是一个只针对位置 (x,y,z) 的数值逆运动学求解器
    % 它可以替代 IKinBody，用于自由度不足 (Under-actuated) 的机器人
    ...
    for i = 1:max_iter
        ...
        J_pos = Js(4:6, :); % 只取雅可比矩阵的线速度部分 (后三行)
        
        % 4. 牛顿-拉夫森迭代: q_new = q + pinv(J) * error
        theta = theta + pinv(J_pos) * err;
    end
end